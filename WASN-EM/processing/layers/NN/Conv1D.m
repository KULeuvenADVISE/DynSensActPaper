...
function [output_shape, complexity, nr_parameters] = Conv1D(pp,gp,input_shape)
    ...
    % output shape
    if strcmp(pp.padding,'same'), P = (pp.kernel_size-1)/2; else, P = [0]; end; 
    output_shape(1,[gp.chid gp.featid gp.frameid]) = [pp.filters 1 (input_shape(gp.frameid)-pp.kernel_size+2*P)./pp.strides+1]; %get output shape
