...
function [output_shape, complexity, nr_parameters] = MaxPooling2D(pp,gp,input_shape)
    ...
    % output shape
    if strcmp(pp.padding,'same'), P = (pp.pool_size-1)/2; else, P = [0 0]; end;    
    output_shape(1,[gp.chid gp.featid, gp.frameid]) = [input_shape(gp.chid) (input_shape([gp.featid, gp.frameid])-pp.pool_size+2*P)./pp.strides+1]; %get output shape
