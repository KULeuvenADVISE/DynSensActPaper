...
function [output_shape, complexity, nr_parameters] = Conv2D(pp,gp,input_shape)
    ...
    % output shape
    if strcmp(pp.padding,'same'), P = (pp.kernel_size-1)/2; else, P = [0 0]; end;
    output_shape(1,[gp.chid gp.featid gp.frameid]) = [pp.filters (input_shape([gp.featid, gp.frameid])-pp.kernel_size+2*P)./pp.strides+1]; %get output shape
